...
//App::uses("OAuthSimple", "Component");
...
//class Trello
{
    ...
    public function authorize($userOptions = array(), $return = false)
    {
        ...
        if (isset($_GET['oauth_verifier'], $_SESSION['oauth_token_secret'])) {
            debug("back from an authorization request ");
            ...
            if (empty($returned_items['oauth_token'])) {               
        ...
        $ch = curl_init(); // init
        curl_setopt($ch, CURLOPT_URL, $request['signed_url']); // URLをセット
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
        curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,  false);
        $result = curl_exec($ch);
